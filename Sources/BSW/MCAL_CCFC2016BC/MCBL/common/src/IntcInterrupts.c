...
* DESCRIPTION : Contains an implementations of generic interrupt .
                controller handling routines for the CCFC2016BC MCU families
...
#include "CCFC2016BC.h"         /* MCU platform development header       */
...
#define MAKE_HLI_ADDRESS(hli_name, c_expr) enum { hli_name = ((int)(c_expr))}; /* PRQA S 5209 */

/** Address of the IACKR Interrupt Controller register. */
MAKE_HLI_ADDRESS(INTC_IACKR, &INTC.IACKR.R)/* PRQA S 3432, 3205, 0589 */
/** Address of the EOIR End - of - Interrupt Register register. */
MAKE_HLI_ADDRESS(INTC_EOIR, &INTC.EOIR.R)/* PRQA S 3432, 3205, 0589 */

/** Address of the MCR-- used for e200z0h initialization */
MAKE_HLI_ADDRESS(INTC_MCR, &INTC.MCR.R)/* PRQA S 3432, 3205, 0589 */
...
#define INTC_INTERRUPTS_REQUEST_VECTOR_TABLE_SIZE (479 * 4)
...
/* The INTC vector table will be placed in RAM.
   ...
   initialized to zero.
*/
INTCInterruptFn INTCInterruptsHandlerTable[INTC_INTERRUPTS_REQUEST_VECTOR_TABLE_SIZE]; /* PRQA S 3408, 1514 */
#pragma pop
#endif
...
{
    INTCInterruptFn *handlerFn = (INTCInterruptFn*)(*(unsigned int*)&INTC.IACKR.R); /* PRQA S 3678, 0312, 5209 */
...
__asm void INTC_InitINTCInterrupts(void)/* PRQA S 1006 */
{
    nofralloc

    /* Set the location of the ISR Handler Table in INTC IACKR Register */
    lis r0, IntcIsrVectorTable@h/* PRQA S 0289 */
    ori r0, r0, IntcIsrVectorTable@l
